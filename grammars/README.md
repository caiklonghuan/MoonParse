...
对于 C、Python、MoonBit 这类语言，当前的目标并不是在 `grammars/` 中完整复制官方规范，而是提供足够稳定的示例语法、有代表性的结构，以及能覆盖查询、高亮、恢复、类型生成测试的核心子集。这使得 `grammars/` 更像一个项目内建样本库，而不是正式发布的完整语言前端集合。
...
- 为 `runtime/`、`query/`、`typegen` 编写端到端示例与回归测试；
