...
MoonBit 编写的解析器生成器与语言工具链基础设施。当前仓库已经打通了 Grammar DSL、ParseTable 生成、GLR 运行时、增量解析、错误恢复、查询 / 高亮、WASM 桥接和 website playground ，适合拿来做内置语法验证、浏览器集成、命令行调试和语言工具链实验。
...
- website 侧的 Python 预编译恢复路径已经能保住 builtin identity，函数体 / 类体能进入真实语句节点，例如 `if_statement`、`return_statement`、`for_statement`、`class_statement`。
...
如果目标不是直接跑 website，而是把解析器当成可分发能力嵌入浏览器、Node.js、Electron 或其他 JS 宿主，当前推荐两条路径：
...
建表层选择 LALR，是为了让 ParseTable 更紧凑、工程上更容易调试和序列化；运行时选择 GLR，是为了让语法作者不必在项目一开始就把所有冲突都压到“完全确定”。

#### 3. 默认走数据驱动运行时，而不是直接代码生成 parser

仓库当前主路径是 `Grammar -> ParseTable -> runtime`。这样做更适合 wasm 分发和缓存，也更适合 `moon run cmd/main --`、脚本和宿主环境共享同一份表产物。

#### 4. CST first，而不是一上来做 AST first

MoonParse 的 runtime 优先保证 CST 可用，因为查询、高亮、游标、增量复用和错误恢复都直接依赖树的结构与位置信息。AST 可以在上层继续做，但不应反过来约束底层 parse runtime 的输出形态。

#### 5. 错误恢复返回可消费的树，而不是简单 hard fail

对编辑器和 website 来说，输入一半经常是不完整的。MoonParse 的恢复目标是尽量产出带 `ERROR` / `MISSING` 的可遍历树，让 query / highlighter / cursor 还能继续工作，并把真正的运行时异常和普通语法错误区分开。

#### 6. 增量解析复用旧树，而不是每次全量重跑

运行时通过旧树和 `InputEdit` 重用未受影响的子树。这样做是为了让 website playground 在连续输入时仍可交互，也让后续编辑器集成和一致性脚本复用同一条 runtime API。

#### 7. 缩进敏感语言依赖 stateful scanner

Python 这类语法不能只靠普通 extras 或浅层缩进文本块解决。当前设计已经明确把 stateful scanner 视为 runtime 一等能力，而不是 website 的局部补丁。

#### 8. WASM 边界使用句柄模型

MoonBit 对象不会直接暴露给 JS。`wasm/` 通过 parser/tree/query/cursor 的整数句柄跨边界传递状态，确保生命周期可控、宿主侧无需理解内部对象布局、website / Node / browser 复用同一套桥接思路。
...
MoonParse 当前不是“只展示一个 parser demo”的仓库，而是一条从 Grammar DSL 到 runtime / query / wasm / website 的完整工程链。你可以把它当成：

- MoonBit 原生的语言工具链实验平台；
- 浏览器侧可分发的解析器运行时；
- 用来验证 grammar、恢复、增量解析和查询能力的统一仓库。
